namespace :db do
  namespace :mhgen do
    ...
    task :generate_seeds => :environment do
      ...
      quests_query = 'select distinct quests._id, quests.name, quests.rank, quests.goal from quests;'
      ...
      @db_file = "#{Rails.root.to_s}/db/mhgen.db"
...
MonsterSet.create(id: 3, name: "Monster Hunter Generations", url: 'mhgen', enabled: true, default: true)
Rank.create(id: 7, monster_set_id: 3, name: "Low")
Rank.create(id: 8, monster_set_id: 3, name: "High")
Rank.create(id: 9, monster_set_id: 3, name: "G")
      EOF
      ...
      CSV.parse(run(items_query)) do |(i_id, name)|
        items_count += 1
        output.puts "items_lookup[#{i_id}] = Item.new(name: \"#{name}\", rank_id: 7)"
      end
      ...
      quests_count = 0
      CSV.parse(run(quests_query)) do |(q_id, name, rank, goal)|
        quests_count += 1
        rank = real_rank(rank)
...
Generated seeds for MH Generations database at #{@db_file}
